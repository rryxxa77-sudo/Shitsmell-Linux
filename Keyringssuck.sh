...
# Internet Setup TUI (Fixed: Uses WIFI_PASS to avoid overwriting user PASS)
...
arch-chroot /mnt /bin/bash <<EOF
    ...
    # --- CACHYOS REPO SETUP (REINFORCED WITH RETRIES & KEY FIX) ---
    ...
    while [ \$ATTEMPT -lt 10 ]; do
        ...
        cd /tmp
        rm -rf cachyos-repo*
        if curl -sSL https://mirror.cachyos.org/cachyos-repo.tar.xz -o cachyos-repo.tar.xz; then
    ...
    if [ "\$SUCCESS" = false ]; then
        echo "ERROR: CachyOS repo setup failed."
        exit 1
    fi

    # Fix Keyring specifically for CachyOS
    ...
    if [[ -z "\$ROOT_PARTUUID" ]]; then
        echo "ERROR: PARTUUID lookup failed."
        exit 1
    fi
