...
arch-chroot /mnt /bin/bash <<EOF
    ...
    # --- BOOTLOADER REINFORCED (FIXED TYPO & BLKID) ---
    ...
    if [[ -z "\$ROOT_PARTUUID" ]]; then
        echo "ERROR: Could not get PARTUUID for \$P2 inside chroot."
        exit 1
    fi

    # Fixed typo: referencing ROOT_PARTUUID consistently
    [[ "\$FS_TYPE" == "btrfs" ]] \
...
EOT
    ...
    if [[ ! -f /boot/loader/loader.conf ]] || [[ ! -f /boot/loader/entries/arch.conf ]]; then
        echo "ERROR: Bootloader config files were NOT created!"
        exit 1
    fi
