#!/bin/bash
# SHITSMELL LINUX DEPLOYMENT SCRIPT - REINFORCED V4
set -eo pipefail
...
arch-chroot /mnt /bin/bash <<EOF
    ...
    cd cachyos-repo
    # Ignore exit code 1 if repo exists
    yes | ./cachyos-repo.sh || true
    
    echo "Nuking keyrings to ensure CachyOS trust..."
    pacman-key --init
    pacman-key --populate archlinux cachyos
    pacman -Sy --noconfirm cachyos-keyring cachyos-mirrorlist cachyos-v3-mirrorlist

    # PRE-EMPTIVE MESA STRIKE
    pacman -Rdd --noconfirm mesa 2>/dev/null || true

    # Use --needed to ignore 'local is newer than repo' warnings
    ...
    flatpak install -y flathub com.usebottles.bottles com.dec05eba.gpu_screen_recorder
    flatpak install -y trinity com.trench.trinity.launcher
    flatpak install -y flathub io.qt.qtwebengine.BaseApp//6.10 org.kde.Platform//6.10

    # --- BOOTLOADER REINFORCED ---
    echo "Installing systemd-boot..."
    mkdir -p /boot/loader/entries
    bootctl install

    case "\$KERN_PKG" in
        "linux")      VML="vmlinuz-linux";      INT="initramfs-linux.img" ;;
        "linux-zen")  VML="vmlinuz-linux-zen";  INT="initramfs-linux-zen.img" ;;
        "linux-lts")  VML="vmlinuz-linux-lts";  INT="initramfs-linux-lts.img" ;;
        *)            VML="vmlinuz-\$KERN_PKG";  INT="initramfs-\$KERN_PKG.img" ;;
    esac

    UCO=\$(grep -q "GenuineIntel" /proc/cpuinfo && echo "intel-ucode.img" || echo "amd-ucode.img")
    ROOT_PARTUUID=\$(blkid -s PARTUUID -o value \$P2)
    
    [[ -z "\$ROOT_PARTUUID" ]] && { echo "ERROR: PARTUUID lookup failed."; exit 1; }

    [[ "\$FS_TYPE" == "btrfs" ]] \
        && OPT="root=PARTUUID=\$ROOT_PARTUUID rootflags=subvol=@ rw quiet" \
        || OPT="root=PARTUUID=\$ROOT_PARTUUID rw quiet"
...
EOT
    ...
    [[ -f /etc/systemd/system/lactd.service ]] && systemctl enable lactd
...
ui_banner
gum style --foreground 46 "Shitsmell Linux deployment complete."
